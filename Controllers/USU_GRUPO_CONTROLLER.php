...
//esta función asigna los valores que vinieron del formulario al modelo USU_GRUPO_Model
function get_data_form(){
	...
	$USU_GRUPO = new USU_GRUPO_Model(
		$login,
		$IdGrupo   
	);//instancia un objeto de la clase modelo USU_GRUPO_Model
	
	return $USU_GRUPO; //devuelve un objeto del modelo USU_GRUPO_Model
}
...
switch ( $_REQUEST[ 'action' ] ) {
	case 'ADD': //se hace este case en el caso de que queramos insertar
		if ( !$_POST ) { // si no existe dolar POST  se muestra la vista ADD de USU_GRUPO.
			...
			$USU_GRUPO = get_data_form();// se pasa a la variable USU_GRUPO un objeto del modelo USU_GRUPO_Model
			$respuesta = $USU_GRUPO->ADD();//obtenemos la respuesta que viene del método ADD() de la clase USU_GRUPO_Model
	...
	case 'SEARCH': //se hace este case en el caso de que queramos buscar
		if ( !$_POST ) { // si no existe dolar POST  se muestra la vista SEARCH de USU_GRUPO.
			...
			$USU_GRUPO = get_data_form(); //se pasa a la variable USU_GRUPO un objeto del modelo USU_GRUPO_Model
			$datos = $USU_GRUPO->SEARCH();//obtenemos la respuesta que viene del método SEARCH() de la clase USU_GRUPO_Model
	...
	case 'DELETE': //se hace este case en el caso de que queramos eliminar
		if ( !$_POST ) { // si no existe dolar POST  se muestra la vista DELETE  de USU_GRUPO con todos sus valores.
			$USU = new USU_GRUPO_Model( $_REQUEST[ 'login' ], $_REQUEST[ 'IdGrupo' ] ); //en $USU se le pasará un login y un IdGrupo elegido en la vista de SHOWALL.
			$valores = $USU->RellenaDatos( $_REQUEST[ 'login' ], $_REQUEST[ 'IdGrupo' ]);//con el método RellenaDatos pasaremos el valor de login y de IdGrupo
			new USU_GRUPO_DELETE( $valores ); //se muestra la vista DELETE con el login y el IdGrupo.
		} else {//si existe dolar POST
			$USU = get_data_form(); // se le pasa a la variable $USU el login y IdGrupo a eliminar
			$respuesta = $USU->DELETE(); // con el método DELETE de USU_GRUPO_Model se elimna ese login y IdGrupo de la base de datos.
	...
	case 'SHOWCURRENT': //se hace este case en el caso de que queramos  ver una tupla en detalle.
		$USU_GRUPO = new USU_GRUPO_Model( $_REQUEST[ 'login' ], $_REQUEST[ 'IdGrupo' ]);//en $USU se le pasará un login y un IdGrupo elegido en la vista de SHOWALL.
	...
	default: // por defecto aparecerá la vista SHOWALL.
		if ( !$_POST ) {//si ni existe deolar POST
			$USU_GRUPO = new USU_GRUPO_Model( '', '');//se crea una instancia de la clase USU_GRUPO_Model con parametros vacíos para que nos coga todas las tuplas de la base de datos.
		} else {//si existe dolar POST
